...
def parse_arguments(config) :
    ...
    parser.add_argument("--no_summary",  action='store_false', help="Do not display a summary table.")
    ...
    parser.add_argument("-o", "--order",          type=str, choices=config["order_parameters"],  help="order result.")
...
def render(a, filename, format, no_space, headers, tblfmt, index, quiet):
    formatters = {
        "txt": lambda: a.to_string(filename, index=index),
        "csv": lambda: a.to_csv(filename, index=index),
        "json": lambda: a.to_json(filename, index=index),
        "tab": lambda: tabulate(a, headers=headers, tablefmt=tblfmt, showindex=index)
    }
...
def parse_data(fp, keywords, options):
    ...
    files = []
    if os.path.isdir(fp) :
        for racine, dirs, fichiers in os.walk(fp):
...
def visualise(keywords, results, args):
    ...
    render(df_results[args.parameters], args.extract, args.render, args.no_space, df_results.columns.tolist(), args.tblfmt, args.index, args.quiet)
