...
def deobfuscate_format(line):
    ...
    while current_str != previous_str:
        ...
        matches = re.finditer(pattern_find, line)
        if matches : 
            for match in matches : 
                format_string = match.group(1)
                arguments = [argument.strip().replace('"', "").replace("'", "") for argument in match.group(2).split(",")]
                
                try:
                    res = format_string.format(*arguments)  
                    line = line.replace(match.group(),res)
                except IndexError:
                    found_recursion = True
                    
                    # while found_recursion : 
                    recursion_matches = re.search(recursion_pattern, line[match.start():])
                    if recursion_matches :
                        subresult = re.search(pattern_find, line[match.end():])

                        if subresult : 
                            try : 
                                format_string = subresult.group(1)
                                arguments = [argument.strip().replace('"', "").replace("'", "") for argument in subresult.group(2).split(",")]
                                res = format_string.format(*arguments)  
                                line = line.replace(subresult.group(),res)
                            except : 
                                pass
                    else : 
